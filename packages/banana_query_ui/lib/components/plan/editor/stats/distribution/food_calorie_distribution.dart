...
class FoodCalorieDistribution extends StatefulWidget {
  const FoodCalorieDistribution({super.key, required this.foods});
...
class FoodCalorieDistributionState extends State<FoodCalorieDistribution>
    with CalorieDistributionMixin {
  ...
  List<ScatterSpot> getSpots() {
    return foods.map((e) {
      ...
      return FoodSpot(
          label, calories.amount.toDouble() ?? 0.0, location.x, location.y,
          ...
          scatterTouchData: ScatterTouchData(
              ...
              touchTooltipData: ScatterTouchTooltipData(
                getTooltipItems: (spot) {
                  if (spot is FoodSpot) {
                    return ScatterTooltipItem("${spot.calories} kcal",
                        textStyle: const TextStyle(color: Colors.white));
                  }
                  return ScatterTooltipItem("$spot",
                      textStyle: const TextStyle(color: Colors.white));
                },
              )),
